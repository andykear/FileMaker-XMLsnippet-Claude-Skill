...
| Version | Notes |
|---------|-------|
| 1.14 | Boolean polarity corrected. A state attribute carries the raw stored bit, not the meaning of the element's name, so <NoInteract state="True"/> is "With dialog: Off". <Option> documented as a generic element whose meaning depends on the step, not a dialog setting: skip data entry validation on Commit Records, Open file (inverted) on Convert File, password on Print PDF. |
